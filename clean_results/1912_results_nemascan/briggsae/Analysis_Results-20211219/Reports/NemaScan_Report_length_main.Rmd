---
title: "NemaScan Report for length"
...
whether_mediate <- ifelse("false" == "true", TRUE, FALSE)
...
processed_mapping <- read.delim("processed_length_AGGREGATE_mapping.tsv", stringsAsFactors=FALSE) %>%
...
BF.frame <- processed_mapping %>%
  ...
  dplyr::mutate(BF = BF, EIGEN  = EIGEN, user = unique(processed_mapping$BF))
...
for (i in seq_len(nrow(QTL_peaks))) { 
    ...
    out <- c(out, knitr::knit_child('NemaScan_Report_region_length.Rmd'))
}
