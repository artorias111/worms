...
# if finemap region is too large, don't plot plotly
...
Mediation analysis was performed to analyze if gene expression variation is significantly correlated with the phenotype (overlap of phenotype QTL with expression QTL). Top candidates whose expression might mediate the phenotype QTL are shown below. (*Note: expression data currently unpublished*)
...
if(whether_mediate) {
    # plot mediation
    # plot
    summarized_med <- readr::read_tsv("input.19")
...
```{r, out.width = '100%', fig.height=30, fig.width=20, eval = div}

# genome bin table
all_QTL_bins <- read.delim("input.9", header=FALSE, stringsAsFactors=FALSE)%>%
...
if (file.info("input.10")$size!=0) {
  
  all_QTL_div <- read.delim("input.10", header=FALSE, stringsAsFactors=FALSE) %>%
...
ref_alt_div %>%
  dplyr::left_join(ref_alt) %>%
  ggplot2::ggplot(.) +
  aes(x = start, xend = end, yend = strain, y = strain) +
  ...
  scale_color_manual(values = c("1" = "indianred1","0" = "grey90","-1" = "dodgerblue3"), 
                     labels = c("Divergent from N2", "Not divergent", "No Divergent Data"),
                     name = "") +
...
print(glue::glue("Divergent regions not yet calculated for {species}"))
split_by_div = FALSE
...
<br>
```{r, fig.height=30, fig.width=20, eval = div}
# if (file.info("all_QTL_div.bed")$size!=0) {

haplotype_in_QTL_region <- read.delim("input.11", header=FALSE, stringsAsFactors=FALSE) %>% 
    dplyr::select(-V12)
...
print(glue::glue("Haplotypes not yet calculated for {species}"))
